...
class ImageClozeAssociationComponent extends React.Component {
  constructor(props) {
    super(props);
    const { model: { possibleResponses, responseContainers, shuffle } } = props;
    const possibleResponsesParsed = shuffle ? shuffleArray(possibleResponses) : possibleResponses;

    this.state = {
      answers: [],
      draggingElement: { id: '', value: '' },
      possibleResponses: possibleResponsesParsed.map(item => ({ id: generateId(), value: item })),
