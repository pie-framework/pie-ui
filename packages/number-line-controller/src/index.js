...
export function model(question, session, env) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (model.config) {
      ...
      let exhibitOnly = question.model.config
        ? question.model.config.exhibitOnly
        : null;
      let disabled = env.mode !== 'gather' || exhibitOnly === true;

      let feedback =
        evaluateMode && getFeedback(correctness, question.feedback);
